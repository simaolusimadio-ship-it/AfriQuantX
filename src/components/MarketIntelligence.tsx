    ...
    <div className="flex-1 bg-black border border-white/10 rounded-3xl flex flex-col overflow-hidden relative shadow-2xl animate-in fade-in slide-in-from-bottom-4 duration-500 text-white">
      ...
      <div className="p-6 border-b border-white/10 bg-white/[0.01] flex items-center justify-between relative z-10">
        ...
        <div className="flex gap-2">
          <button className="px-4 py-2 bg-black border border-white/10 rounded-xl text-xs uppercase tracking-wider font-bold text-white hover:border-[#D4AF37]/50 transition-colors flex items-center gap-2">
          ...
          <>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              {data.overview?.map((stat: any, idx: number) => (
                <div key={idx} className="p-5 rounded-2xl bg-black border border-white/10 flex flex-col gap-2 hover:border-[#D4AF37]/40 transition-colors">
                  ...
                  <div className="text-xs font-bold font-mono flex items-center gap-1 text-[#D4AF37]">
                    {stat.up === true && <TrendingUp className="w-3 h-3 text-[#D4AF37]" />}
                    {stat.up === false && <TrendingDown className="w-3 h-3 text-[#D4AF37]" />}
                    {stat.up === null && <AlertTriangle className="w-3 h-3 text-[#D4AF37]" />}
                    {stat.change}
                  </div>
                </div>
              ))}
            ...
            <div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                {data.summaries?.map((summary: any, idx: number) => (
                  <div key={idx} className="p-5 rounded-2xl border border-white/10 bg-black relative overflow-hidden hover:border-[#D4AF37]/40 transition-colors">
                    <div className="flex justify-between items-start mb-3">
                      <div className="flex items-center gap-2">
                        <span className="px-2 py-1 rounded-md text-xs font-bold font-mono border bg-[#D4AF37]/10 text-[#D4AF37] border-[#D4AF37]/20">
            ...
            <div className="p-6 rounded-2xl bg-black border border-white/10">
              ...
              <div className="space-y-4">
                {data.trends?.map((item: any, idx: number) => (
                  <div key={idx} className="flex items-center justify-between p-4 rounded-xl bg-white/[0.02] border border-white/10 hover:border-[#D4AF37]/30 transition-colors">
                    <div>
                    ...
                    <div className="flex gap-6 text-right">
                      <div>
                        ...
                        <div className="text-sm font-bold text-[#D4AF37] font-mono">{item.confidence}</div>
                      </div>
                      <div>

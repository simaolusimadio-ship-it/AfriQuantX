...
import { Play, Pause, Volume2, VolumeX, Maximize2, Sparkles, ShieldCheck, Cpu, ArrowUpRight, CheckCircle2 } from 'lucide-react';
...
export function PlatformVideoPlayerSection({
  ...
  return (
    <div className="relative w-full max-w-5xl mx-auto my-12 px-4 sm:px-6">
      {/* Glow Backdrop */}
      <div className="absolute -inset-2 bg-gradient-to-r from-blue-600/20 via-emerald-500/20 to-lime-400/20 rounded-[36px] blur-2xl opacity-70 pointer-events-none" />

      {/* Main Container */}
      <div className="relative bg-zinc-950 rounded-[32px] border border-white/10 shadow-2xl overflow-hidden space-y-0">
        ...
        <div className="relative aspect-video w-full bg-black overflow-hidden group">
          ...
          <div className="absolute top-4 left-4 right-4 flex items-center justify-between z-20 pointer-events-none">
            ...
            <div className="hidden sm:inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-black/80 backdrop-blur-md border border-white/10 text-xs font-mono font-bold text-[#D5FF2F]">
              ...
              <span>AQEI ENGINE v4.2 DEMO</span>
            </div>
          </div>
        ...
        <div className="p-6 md:p-8 bg-zinc-900/90 border-t border-white/10 flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
          <div className="space-y-1">
            <div className="text-xs font-mono text-blue-400 font-bold uppercase tracking-wider flex items-center gap-1.5">
              <Sparkles className="w-3.5 h-3.5" />
              <span>EXECUTIVE DEMO & TECHNICAL OVERVIEW</span>
            </div>
            <h4 className="text-xl font-bold text-white tracking-tight">{title}</h4>
            <p className="text-xs text-zinc-400 max-w-2xl leading-relaxed">{subtitle}</p>
          </div>

          <div className="flex items-center gap-3">
            <div className="px-4 py-2 rounded-xl bg-zinc-950 border border-zinc-800 text-xs font-mono text-emerald-400 flex items-center gap-2">
